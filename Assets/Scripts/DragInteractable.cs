...
public class DragInteractable : XRBaseInteractable
{
    ...
    IEnumerator CalculateDrag ()
    {
        while (m_interactor != null)
        {
            ...
            Debug.Log("CalculateDrag is running");
            Debug.Log($"Interactor Position: {m_interactor.transform.position}");

            Debug.Log($"Drag Percent: {dragPercent}");
            Debug.Log($"Start Position: {startDragPosition.localPosition}");
            Debug.Log($"End Position: {endDragPosition.localPosition}");
